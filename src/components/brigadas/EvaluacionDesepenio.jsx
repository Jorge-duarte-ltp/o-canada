...
const EvaluacionDesepenio = ({ data, backTable, setReload, reload }) => {
  ...
  const sumarPuntos = () => {
    const aptitud_fisica = state.aptitud_fisica
    ...
    const productividad = state.productividad
      ? parseInt(state.productividad)
      : 0;
    const liderazgo = state.liderazgo ? parseInt(state.liderazgo) : 0;
    const licencia_manejo = state.licencia_manejo
      ? parseInt(state.licencia_manejo)
      : 0;
    const estado_salud = state.estado_salud ? parseInt(state.estado_salud) : 0;
    const exp_incendios_forestales = state.exp_incendios_forestales
      ? parseInt(state.exp_incendios_forestales)
      : 0;
    const sis_comando_incendios = state.sis_comando_incendios
      ? parseInt(state.sis_comando_incendios)
      : 0;
    const cadena_mando = state.cadena_mando ? parseInt(state.cadena_mando) : 0;
    const idioma_ingles = state.idioma_ingles
      ? parseInt(state.idioma_ingles)
      : 0;
    const equipo_despliegue = state.equipo_despliegue
      ? parseInt(state.equipo_despliegue)
      : 0;
    const uso_markIII = state.uso_markIII ? parseInt(state.uso_markIII) : 0;
    const uso_motosierra = state.uso_motosierra
      ? parseInt(state.uso_motosierra)
      : 0;
    const capacidad_gestion = state.capacidad_gestion
  ...
  return (
    <Fragment>
      ...
      <div className="row body_wrap">
        ...
        <div className="col-12 pt-4">
          <label>1.-LICENCIA DE MANEJO</label>
          <p>¿Demostró la capacidad para conducir en el extranjero?</p>
          <SelectCalificacion
        ...
        <div className="col-12 pt-4">
          <label>2.- ESTADO DE SALUD</label>
          <p>¿Mantuvo un estado de salud adecuado durante el despliegue?</p>
          <SelectCalificacion
        ...
        <div className="col-12 pt-4">
          <label>4.- PRODUCTIVIDAD</label>
          <p>
            Habilidad para cumplir los objetivos. ¿Concretó las asignaciones de
            manera adecuada y en los tiempos requeridos?
          </p>
          <SelectCalificacion
        ...
        <div className="col-12 pt-4">
          <label>6.- CADENA DE MANDO</label>
          <p>
            ¿Utilizó los canales establecidos y respeto la línea de
            comunicación?
          </p>
          <SelectCalificacion
        ...
        <div className="col-12 pt-4">
          ...
          <input className="form-control" disabled value={sumatoria} />
        </div>
        <div className="col-12 pt-4">
